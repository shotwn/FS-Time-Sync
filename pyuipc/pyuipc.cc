...
#if PY_MAJOR_VERSION>=3
                item = PyUnicode_FromString(reinterpret_cast<const char*>(data + dataOffset));
#else
                item = PyString_FromString(reinterpret_cast<const char*>(data + dataOffset));
#endif
            } else {
#if PY_MAJOR_VERSION>=3
                item = PyUnicode_FromStringAndSize(reinterpret_cast<const char*>(data + dataOffset),
...
#endif
                setError(PyExc_TypeError, const_cast<char*>("a string is expected for generic byte-arrays and strings: offset=0x%04x, type=%s, typeInfo: letter='%c', length=%u, alignment=%u"),
...
#if PY_MAJOR_VERSION>=3
                memcpy(data, PyUnicode_AS_DATA(object),
                       min(info.typeInfo->length,
                           static_cast<size_t>(PyUnicode_GET_DATA_SIZE(object))));
#else
